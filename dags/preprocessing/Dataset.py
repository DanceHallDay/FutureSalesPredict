...
class Dataset(IDataset):
    ...
    def get_data(self, add_cat_preprocessing: bool = True, add_lag_features: bool = False, *args, **kwargs) -> pd.DataFrame:
        result_df = self.df.drop(
            ['item_cnt_month', 'item_price', 'item_revenue'], axis=1) if self.train else self.df
    ...
    def get_labels(self) -> pd.Series | None:
    ...
    def __cat_features_preprocessing__(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['item_fixed_category'] = df['item_category_id'].apply(
    ...
    def __etl__(self, file_path: str, add_item_cartesian_product: bool = False) -> pd.DataFrame:
        ...
        if add_item_cartesian_product and self.train:
            ...
            for month in range(34):
                unique_shops = dataset.loc[
                    dataset['date_block_num'] == month,
                    'shop_id'
                ].unique()
                unique_items = dataset.loc[
                    dataset['date_block_num'] == month,
                    'item_id'
                ].unique()
            ...
            dataset = pd.merge(grid, dataset, how='left', on=[
                               'shop_id', 'item_id', 'date_block_num']).fillna(0)
        ...
        dataset = pd.merge(dataset, item_cat,
